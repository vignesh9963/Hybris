...
{
	protected static final String TEST_TYPECODE = TestItemModel._TYPECODE.toLowerCase();
	...
	public void shouldSeeChangesWhenBothNameAndDescriptionOfTypeHaveCahnged()
	{
		...
		final LocalizationInfo localizationInfo = new TestLocalizationInfo(currentLanguage.getPK(),
				ImmutableMap.<String, String>of(TEST_TYPE_NAME_KEY,
						newName,
						TEST_TYPE_DESCRIPTION_KEY,
						newDescription),
				pkToType);
	...
	public void shouldSeeChangesWhenBothNameAndDescriptionOfAttributeHaveCahnged()
	{
		...
		final LocalizationInfo localizationInfo = new TestLocalizationInfo(currentLanguage.getPK(),
				ImmutableMap.<String, String>of(
						...
						TEST_ATTRIBUTE_DESCRIPTION_KEY,
						newDescription), pkToType);
	...
	public void shouldAllowToOverrideAttributeNameInSubtype()
	{
		...
		final String subtypeNameBefore = typeService.getAttributeDescriptor(SUBTYPE_TYPECODE, SUBTYPE_ATTRIBUTE_QUALIFIER)
		                                            .getName();
		...
		final String subtypeNameAfter = typeService.getAttributeDescriptor(SUBTYPE_TYPECODE, SUBTYPE_ATTRIBUTE_QUALIFIER)
		                                           .getName();
	...
	public void shouldAllowToOverrideAttributeDescriptionInSubtype()
	{
		...
		final String subtypeDescriptionBefore = typeService.getAttributeDescriptor(SUBTYPE_TYPECODE, SUBTYPE_ATTRIBUTE_QUALIFIER)
		                                                   .getDescription();
		final String newSubtypeDescription = concat(subtypeDescriptionBefore, "SUB_CN");

		final LocalizationInfo localizationInfo = new TestLocalizationInfo(currentLanguage.getPK(), ImmutableMap.of(
				TEST_ATTRIBUTE_DESCRIPTION_KEY, newDescription, SUBTYPE_ATTRIBUTE_DESCRIPTION_KEY, newSubtypeDescription),
				pkToType);
		...
		final String subTypeDescriptionAfter = typeService.getAttributeDescriptor(SUBTYPE_TYPECODE, SUBTYPE_ATTRIBUTE_QUALIFIER)
		                                                  .getDescription();
		assertThat(subTypeDescriptionAfter).isNotNull().isNotEqualTo(subtypeDescriptionBefore).isEqualTo(newSubtypeDescription);
	}

	@Test
	public void shouldAttributesLocalizationBeFiltered()
	{
		//given
		registerFilterBean();
		...
		final String newName = concat(nameBefore, "CN");
		final LocalizationInfo localizationInfo = new TestLocalizationInfo(currentLanguage.getPK(),
				ImmutableMap.of(TEST_ATTRIBUTE_NAME_KEY, newName), pkToType);
		...
		//then
		final String nameAfter = typeService.getAttributeDescriptor(TEST_TYPECODE, TEST_ATTRIBUTE_QUALIFIER).getName();
		assertThat(nameAfter).isNotNull().isEqualTo(nameBefore);
	}
	...
	private void enableFiltering(final boolean state)
	{
		Config.setParameter(JdbcBasedTypeSystemLocalization.IS_LOCALIZATION_FILTERING_ENABLED, String.valueOf(state));
	}

	private void registerFilterBean()
	{
		final BeanDefinitionRegistry registry = ((BeanDefinitionRegistry) Registry.getApplicationContext()
		                                                                          .getAutowireCapableBeanFactory());
		final GenericBeanDefinition genericBeanDefinition = new GenericBeanDefinition();
		genericBeanDefinition.setBeanClass(TestTypeCodeLocalizableAttributeRowFilterStub.class);
		registry.registerBeanDefinition("junitLocalizableAttributeRowFilter", genericBeanDefinition);
	}
	...
	private static class TestLocalizationInfo extends PropertiesBasedLocalizationInfo
	{
		...
		public TestLocalizationInfo(final PK languagePK, final Map<String, String> localizations,
		                            final Map<Long, String> pkToType)
		{
