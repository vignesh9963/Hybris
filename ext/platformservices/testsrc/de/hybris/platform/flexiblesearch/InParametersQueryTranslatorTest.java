...
public class InParametersQueryTranslatorTest extends ServicelayerBaseTest
{
	...
	//select {PK} FROM {Product} WHERE {name} IN ( ?verylongidentifiermuchlongerthatthrirtycharacters )
	private final String selectCatalogVersionByPkInQry = "SELECT  lp_t0.ITEMPK  FROM junit_productslp lp_t0 WHERE ((lp_t0.LANGPK =?0 ) AND ( lp_t0.p_name  in (?verylongidentifiermuchlongerthatthrirtycharacters) )) AND (lp_t0.ITEMTYPEPK IN  (?11,?12) )";
	...
	public void shouldAnalyzeAndGenerateWithForInClause()
	{
		...
		final Map<String, List<String>> overMaxParams = ImmutableMap.of("names",
				ImmutableList.of("foo", "bar", "baz", "bazinga"));
	...
	@Test
	public void shouldGenerateTableNameWhichMeetsCharacterLimit()
	{
		// given
		final InParametersQueryTranslator translator = createQueryTranslator(3);
		final Map<String, List<String>> overMaxParams = ImmutableMap.of("verylongidentifiermuchlongerthatthrirtycharacters",
				ImmutableList.of("foo", "bar", "baz", "bazinga"));

		// when
		final List<InParametersQueryTranslator.ExceedingParameter> exceedingParams = translator
				.analyzeQuery(selectCatalogVersionByPkInQry, overMaxParams);

		// then
		assertThat(exceedingParams).hasSize(1);
		assertThat(exceedingParams.get(0).getParam()).isEqualTo("verylongidentifiermuchlongerthatthrirtycharacters");
		assertThat(exceedingParams.get(0).getQryFragments().get(0)).isEqualTo(
				"(?verylongidentifiermuchlongerthatthrirtycharacters)");
		assertThat(exceedingParams.get(0).getExtractedLiterals()).isEmpty();
		...
		// then
		assertThat(withQuery).isEqualTo("with with_tbl_verylongidentifie1 (id) as (select 'foo' from dual UNION \n"
