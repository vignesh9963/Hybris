...
public class ProcessDefinitionBuilder
{
	...
	public static class ActionNodeBuilder extends ProcessDefinitionBuilder
	{
		...
		protected StringBuilder getBody(StringBuilder sb)
		{
			this.parameters.forEach(
					(k, v) -> sb.append("\t\t<parameter name=\"").append(k).append("\" value=\"").append(v).append("\"/>\n"));
			this.transitions.forEach(
					(k, v) -> sb.append("\t\t<transition name=\"").append(k).append("\" to=\"").append(v).append("\"/>\n"));
			return sb;
		}
	}
	...
	public static class WaitNodeBuilder extends ProcessDefinitionBuilder
	{
		...
		{
			super("\t<wait id=\"" + id + "\" then=\"" + then + "" + prependProcessCode.map(b -> "\" prependProcessCode=\"" + b)
			                                                                          .orElse("") + "\">\n", "\t</wait>\n");
			this.setParent(Optional.of(parent));
		}
